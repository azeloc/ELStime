...
da_report <- readr::read_delim("data-raw/idb/raw_data/cv88on.txt") |>
  dplyr::mutate(
    ...
    disposition_date = lubridate::mdy(TDATEUSE), # Use TDATEUSE for AO published reports
    filing_date = lubridate::mdy(FDATEUSE), # Use FDATEUSE for AO published reports
    time = ifelse(
      STATUSCD == "L",
      as.numeric(disposition_date - filing_date),
      as.numeric(as.Date("2025-07-25") - filing_date)
    ),
    evento = STATUSCD == "L",
    court_action = !(PROCPROG %in% c(1)),
    evento_competing = factor(
      dplyr::case_when(
    ...
    JURIS = (stringr::str_detect(
      PLT,
      "COMMONWEALTH|USA|UNITED +STATES|STATE +OF|FEDERAL TRADE COMMISSIO"
    ) |
      stringr::str_detect(
        DEF,
        "COMMONWEALTH|USA|UNITED +STATES|STATE +OF|FEDERAL TRADE COMMISSIO"
